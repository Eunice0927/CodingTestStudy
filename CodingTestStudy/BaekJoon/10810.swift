...
//  CodingTestStudy
//
//  Created by Eunsu JEONG on 2023/10/20.
//
...
func quiz10810() {
    ...
    let basketNo: Int = Int(nAndM[0])!
    let shootBall: Int = Int(nAndM[1])!
    var basketArray: [Int] = Array(repeating: 0, count: basketNo)
    
    for _ in 0..<shootBall {
        let shootPlan = readLine()!.split(separator: " ")
        let startBasket: Int = Int(shootPlan[0])! - 1
        let lastBasket: Int = Int(shootPlan[1])! - 1
        let ballNumber: Int = Int(shootPlan[2])!
        for j in startBasket...lastBasket {
